...
# Adjust the system path to import the AHP class
...
def save_input():
    ...
    try:
        ...
        logger.info("Kriteria dan alternatif berhasil diatur.")
...
@app.route('/api/get-input', methods=['GET'])
def get_input():
...
@app.route('/api/compare_alternatives', methods=['POST'])
...
@app.route('/api/compare_alternatives_bulk', methods=['POST'])
...
def calculate():
    try:
        ...
        for crit in ahp.criteria:
            if crit == ahp.criteria[0]:
                CR = ahp.criteria_CR
            else:
                CR = ahp.alternative_CR.get(crit, 1.0)
            if CR <= 0.1:
...
def export_excel():
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # Criteria Weights
            if res.get('criteria_weights'):
                df_weights = pd.DataFrame(res['criteria_weights'])
                df_weights.to_excel(writer, sheet_name='Bobot Kriteria')
